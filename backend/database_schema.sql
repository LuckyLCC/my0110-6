...
CREATE TABLE IF NOT EXISTS users (
    ...
    role ENUM('USER','STAFF') DEFAULT 'USER' COMMENT '用户角色: USER-普通用户, STAFF-商家',
...
CREATE TABLE IF NOT EXISTS booking_orders (
    ...
    price DECIMAL(10,2) NOT NULL COMMENT '价格',
    original_price DECIMAL(10,2) COMMENT '原价',
    status ENUM('PENDING','COMPLETED','CANCELLED','NO_SHOW') DEFAULT 'PENDING' COMMENT '状态: PENDING待核销, COMPLETED已完成, CANCELLED已取消, NO_SHOW未到店',
    payment_status ENUM('UNPAID','PAID') DEFAULT 'UNPAID' COMMENT '支付状态: UNPAID未支付, PAID已支付',
...
CREATE TABLE IF NOT EXISTS payment_orders (
    ...
    price DECIMAL(10,2) NOT NULL COMMENT '价格',
    status ENUM('UNPAID','PAID','CANCELLED') DEFAULT 'UNPAID' COMMENT '状态: UNPAID未支付, PAID已支付, CANCELLED已取消',
    payment_method VARCHAR(50) COMMENT '支付方式',
    payment_time DATETIME COMMENT '支付时间',
    card_start_date DATETIME COMMENT '卡开始日期',
    card_end_date DATETIME COMMENT '卡到期日期',
    transaction_type ENUM('NEW','RENEW') DEFAULT 'NEW' COMMENT '交易类型: NEW-新开卡, RENEW-续费',
    card_status ENUM('INACTIVE','ACTIVE','COMPLETED') DEFAULT 'INACTIVE' COMMENT '卡状态: INACTIVE未生效, ACTIVE生效中, COMPLETED已完成',
...
CREATE TABLE IF NOT EXISTS staffs (
    ...
    status ENUM('ACTIVE','INACTIVE') DEFAULT 'ACTIVE' COMMENT '状态: ACTIVE-激活, INACTIVE-停用',
    last_login_time DATETIME COMMENT '最后登录时间',
    created_at DATETIME NOT NULL,
    updated_at DATETIME NOT NULL
...
CREATE TABLE IF NOT EXISTS invitations (
    ...
    status ENUM('PENDING','ACCEPTED','EXPIRED') DEFAULT 'PENDING' COMMENT '状态: PENDING-待接受, ACCEPTED-已接受, EXPIRED-已过期',
    created_at DATETIME NOT NULL,
