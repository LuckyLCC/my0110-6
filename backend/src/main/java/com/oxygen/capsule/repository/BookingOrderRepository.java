...
public interface BookingOrderRepository extends JpaRepository<BookingOrder, Long> {
    ...
    List<BookingOrder> findByUserIdAndStatusOrderByCreatedAtDesc(Long userId, BookingOrdersStatusEnum status);
    ...
    List<BookingOrder> findByUserIdAndDateAndStatusNot(Long userId, String date, BookingOrdersStatusEnum status);
    // 统计指定购卡记录已核销的预约订单数量
    long countByPaymentOrderIdAndStatus(Long paymentOrderId, BookingOrdersStatusEnum status);
    ...
    List<BookingOrder> findByDateAndTimeSlotAndCabinNameAndSeatNameAndStatusNot(String date, String timeSlot, String cabinName, String seatName, BookingOrdersStatusEnum status);
    
    // 查找某个日期、时段且状态不是已取消的所有预约订单（用于获取已预约的舱位列表）
    List<BookingOrder> findByDateAndTimeSlotAndStatusNot(String date, String timeSlot, BookingOrdersStatusEnum status);
    
    // 查找所有待核销状态的预约订单（用于定时任务检查未到店）
    List<BookingOrder> findByStatus(BookingOrdersStatusEnum status);
}